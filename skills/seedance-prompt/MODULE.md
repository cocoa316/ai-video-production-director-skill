...
| Extend | Continue from accepted source footage only. | Starting from a planned ending or inventing the clip state. | Route to [seedance-continuation](../seedance-continuation/MODULE.md) and use the observed end state. |

## Sequence Boundary

The generic prompt skill must not independently invent continuation state. If the user asks to continue, extend, make part two, or use a previous clip, route to [seedance-continuation](../seedance-continuation/MODULE.md) unless the accepted clip/final frame and observed end state are already present in the sequence state.
...
Before returning, inherit the prompt language selected by the parent Skill. Internal English templates, this module's English wording, and the conversation language never select the final prompt language. Use Chinese by default; switch to English only when the user explicitly requests an English prompt. Preserve quoted dialogue in its spoken language, exact reference tags, proper names, model names, and necessary acronyms. Do not append an unsolicited second-language version.
