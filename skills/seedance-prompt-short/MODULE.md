...
Return one compact prompt in the language selected by the parent Skill. Use Chinese when the user has not specified a prompt language; switch to English only when the user explicitly requests an English prompt. Do not append an unsolicited second-language version. Preserve quoted dialogue in its spoken language, exact reference tags, proper names, model names, and necessary acronyms. Aim for 30-100 English words only when English was selected, or an equivalent compact Chinese length by default. Translate the Director's Read into visible or audible carriers and keep its internal labels out of the prompt. Include a one-line note only if something important was removed.
