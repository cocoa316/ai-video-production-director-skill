...
- **Display type is outlined, never a font stack.** The upstream repository's published wordmark, tagline, and operating-diagram root label use vector glyph outlines. Their website build tooling and source assets are development-only upstream materials and are intentionally not included in this personal runtime Skill.
